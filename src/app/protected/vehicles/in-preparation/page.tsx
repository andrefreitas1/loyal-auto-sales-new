...
export default function InPreparationVehicles() {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar apenas veículos com status 'in_preparation'
        setVehicles(data.filter((v: Vehicle) => v.status === 'in_preparation'));
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-4 sm:py-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold">Veículos em Preparação</h1>
          ...
          >
            <div className="relative aspect-[4/3] sm:h-48">
              ...
              <div className="absolute top-0 right-0 m-2">
                <span className="px-2 py-1 rounded text-xs sm:text-sm font-semibold bg-blue-500 text-white">
                  Em Preparação
                </span>
              </div>
            </div>
      ...
      {vehicles.length === 0 && (
        <div className="text-center py-8 sm:py-12">
          <p className="text-sm sm:text-base text-gray-600">Nenhum veículo em preparação no momento.</p>
          <Link
            href="/protected/vehicles"
