...
export default function AcquiredVehicles() {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar apenas veículos com status 'acquired'
        setVehicles(data.filter((v: Vehicle) => v.status === 'acquired'));
      }
    } catch (error) {
            ...
            <div className="relative h-48">
              {vehicle.images?.length > 0 && !imageErrors[vehicle.id] ? (
                <div className="relative w-full h-full">
                  <Image
                    src={getImageUrl(vehicle.images[0].url)}
                    alt={vehicle.model}
                    fill
